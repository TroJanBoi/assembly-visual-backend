...
// @Failure      500  {object}  map[string]string
// @Security     BearerAuth
// @Router       /google/classroom/courses/{course_id}/confirm [post]
func (gc *GoogleServiceController) ConfirmGoogleClassroomConnectionController(ctx *gin.Context) {
	courseID := ctx.Param("course_id")
	...
	err := gc.googleServiceUsecase.ConfirmGoogleClassroomConnectionUsecase(ctx, userID, courseID, userID)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	ctx.JSON(http.StatusOK, gin.H{"message": "Google Classroom connection confirmed successfully"})
}
...
func (gc *GoogleServiceController) GoogleServiceRegisterRoutes(r gin.IRoutes) {
	...
	r.POST("/classroom/courses/:course_id/confirm", gc.ConfirmGoogleClassroomConnectionController)
}
